...
class PDOCollector extends DataCollector implements Renderable, AssetProvider
{
	...
	protected function collectPDO(TraceablePDO $pdo, TimeDataCollector $timeCollector = null)
	{
		...
		foreach ($pdo->getExecutedStatements() as $stmt) {
			$stmts[] = array('sql'            => $this->renderSqlWithParams ? $stmt->getSqlWithParams($this->sqlQuotationChar) : $stmt->getSql(),
		...
		return array('nb_statements'            => count($stmts),
	...
	{
		return array("database"       => array("icon"    => "inbox",
		                                       "widget"  => "PhpDebugBar.Widgets.SQLQueriesWidget",
		                                       "map"     => "pdo",
		                                       "default" => "[]"),
		             "database:badge" => array("map"     => "pdo.nb_statements",
		                                       "default" => 0));
	}
