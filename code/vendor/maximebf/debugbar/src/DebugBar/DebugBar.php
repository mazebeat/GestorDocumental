...
class DebugBar implements ArrayAccess
{
	...
	{
		$this->data = array('__meta' => array('id'       => $this->getCurrentRequestId(),
	...
	{
		$data = rawurlencode(json_encode(array('id'   => $this->getCurrentRequestId(),
		                                       'data' => $this->getData())));

		if (strlen($data) > $maxTotalHeaderLength) {
			$data = rawurlencode(json_encode(array('error' => 'Maximum header size exceeded')));
		}
