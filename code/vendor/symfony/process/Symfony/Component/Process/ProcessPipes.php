...
class ProcessPipes
{
	...
	public function __construct($useFiles, $ttyMode, $ptyMode = false, $disableOutput = false)
	{
		...
		if ($this->useFiles && !$this->disableOutput) {
			$this->files = array(Process::STDOUT => tempnam(sys_get_temp_dir(), 'sf_proc_stdout'),
			...
			$this->readBytes = array(Process::STDOUT => 0,
			                         Process::STDERR => 0,);
		}
	}
	...
	public function getDescriptors()
	{
		...
		if ($this->ttyMode) {
			return array(array('file', '/dev/tty', 'r'),
				array('file', '/dev/tty', 'w'),
				array('file', '/dev/tty', 'w'),);
		} elseif ($this->ptyMode && Process::isPtySupported()) {
			return array(array('pty'),
				array('pty'),
				array('pty'),);
		}
