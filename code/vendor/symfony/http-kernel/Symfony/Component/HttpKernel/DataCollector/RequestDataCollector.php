...
class RequestDataCollector extends DataCollector implements EventSubscriberInterface
{
	...
	public function collect(Request $request, Response $response, \Exception $exception = null)
	{
		...
		$this->data = array('format'             => $request->getRequestFormat(),
		                    'content'            => $content,
		...
		if (isset($this->controllers[$request])) {
			...
			if (is_array($controller)) {
				try {
					...
					$this->data['controller'] = array('class'  => is_object($controller[0]) ? get_class($controller[0]) : $controller[0],
					                                  'method' => $controller[1],
					                                  'file'   => $r->getFilename(),
					                                  'line'   => $r->getStartLine(),);
				...
				$this->data['controller'] = array('class'  => $r->getName(),
				                                  'method' => null,
				                                  'file'   => $r->getFilename(),
				                                  'line'   => $r->getStartLine(),);
			} else {
