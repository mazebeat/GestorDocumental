...
class ErrorHandler
{
	 ...
	 *
	 * @param int  $level         The level at which the conversion to Exception is done (null to use the
	 *                            error_reporting() value and 0 to disable)
	...
	public function handle($level, $message, $file = 'unknown', $line = 0, $context = array())
	{
		if ($level & (E_USER_DEPRECATED | E_DEPRECATED)) {
			if (isset(self::$loggers['deprecation'])) {
				if (self::$stackedErrorLevels) {
					...
					self::$loggers['deprecation']->warning($message, array('type'  => self::TYPE_DEPRECATION,
					                                                       'stack' => $stack));
				}
		...
		if (isset(self::$loggers['scream']) && !(error_reporting() & $level)) {
			if (self::$stackedErrorLevels) {
				...
				self::$loggers['scream']->log($logLevel, $message, array('type'   => $level,
				                                                         'file'   => $file,
				                                                         'line'   => $line,
	...
	{
		return array(new UndefinedFunctionFatalErrorHandler(),
