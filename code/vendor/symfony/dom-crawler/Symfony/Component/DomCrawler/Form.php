...
class Form extends Link implements \ArrayAccess
{
	 ...
	 * @param string      $method     The method to use for the link (if null, it defaults to the method defined by
	 *                                the form)
	 *
	...
	private function addField(\DOMElement $node)
	{
		...
		} elseif ('input' == $nodeName && !in_array(strtolower($node->getAttribute('type')), array('submit',
					'button',
					'image'))
		) {
	...
	public function getUri()
	{
		...
		if (!in_array($this->getMethod(), array('POST',
					'PUT',
					'DELETE',
					'PATCH')) && $queryString = http_build_query($this->getValues(), null, '&')
		) {
	...
	{
		$this->button = $node;
		if ('button' === $node->nodeName || ('input' === $node->nodeName && in_array(strtolower($node->getAttribute('type')), array('submit',
						'button',
						'image')))
		) {
