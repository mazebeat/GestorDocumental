...
class PHPParser_Lexer_Emulative extends PHPParser_Lexer
{
	...
	public function __construct()
	{
		...
		$newKeywordsPerVersion = array('5.5.0-dev' => array('finally' => PHPParser_Parser::T_FINALLY,
		                                                    'yield'   => PHPParser_Parser::T_YIELD,),
		                               '5.4.0-dev' => array('callable'  => PHPParser_Parser::T_CALLABLE,
		                                                    'insteadof' => PHPParser_Parser::T_INSTEADOF,
		                                                    'trait'     => PHPParser_Parser::T_TRAIT,
		                                                    '__trait__' => PHPParser_Parser::T_TRAIT_C,),
		                               '5.3.0-dev' => array('__dir__'       => PHPParser_Parser::T_DIR,
	...
	protected function postprocessTokens()
	{
		...
		for ($i = 0, $c = count($this->tokens); $i < $c; ++$i) {
			...
			if ('~' === $this->tokens[$i] && isset($this->tokens[$i + 2]) && '~' === $this->tokens[$i + 2] && T_STRING === $this->tokens[$i + 1][0] && preg_match('(^__EMU__([A-Z]++)__(?:([A-Za-z0-9]++)__)?$)', $this->tokens[$i + 1][1], $matches)
				...
				$this->tokens[$i][1] = preg_replace_callback('(~__EMU__([A-Z]++)__(?:([A-Za-z0-9]++)__)?~)', array($this,
						'restoreContentCallback'), $this->tokens[$i][1]);
			}
		}
	}
